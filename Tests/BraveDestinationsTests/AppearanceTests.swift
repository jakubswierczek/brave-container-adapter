...
@Test @MainActor func explicitRenameRemovesLegacySuffixWithoutChangingRouting() async throws {
    let f = try Fixture()
    ...
    let original = try await AppGenerator.generate(configuration: config, receiver: receiver, directory: directory)
    ...
    let updated = try await AppGenerator.generate(configuration: renamed, receiver: receiver, directory: directory, rename: true)
    ...
    #expect(try await AppGenerator.generate(configuration: renamed, receiver: receiver, directory: directory, rename: true) == updated)
}

@Test @MainActor func renameCollisionPreservesBothAppsAndOriginalIcon() async throws {
    let f = try Fixture()
    let directory = f.root.appendingPathComponent("Apps")
    let receiver = URL(fileURLWithPath: "/usr/bin/true")
    let config = try DestinationConfiguration(destination: f.destination, displayName: "Original")
    let app = try await AppGenerator.generate(configuration: config, receiver: receiver, directory: directory, icon: .generated(.work))
    ...
    await #expect(throws: AdapterError.self) {
        try await AppGenerator.generate(configuration: DestinationConfiguration(destination: config.destination, displayName: "Occupied"),
...
@Test @MainActor func importedIconIsStandaloneAndSurvivesNameOnlyUpdate() async throws {
    let f = try Fixture()
    ...
    let directory = f.root.appendingPathComponent("Apps")
    let receiver = URL(fileURLWithPath: "/usr/bin/true")
    let app = try await AppGenerator.generate(configuration: config, receiver: receiver, directory: directory, icon: imported)
...
@Test @MainActor func generatedPresetsAreDistinctAndRestoreTheirSelection() async throws {
    let f = try Fixture()
    ...
    let app = try await AppGenerator.generate(configuration: config, receiver: URL(fileURLWithPath: "/usr/bin/true"),
...
@Test @MainActor func invalidImageAndUnsafeNamesLeaveExistingAppIntact() async throws {
    let f = try Fixture()
    ...
    let directory = f.root.appendingPathComponent("Apps")
    let receiver = URL(fileURLWithPath: "/usr/bin/true")
    let app = try await AppGenerator.generate(configuration: config, receiver: receiver, directory: directory)
    for name in ["../escape", ".hidden", "bad:name", "", "bad\nname", String(repeating: "é", count: 121)] {
        await #expect(throws: AdapterError.self) {
            try await AppGenerator.generate(configuration: DestinationConfiguration(destination: config.destination, displayName: name),
...
@Test @MainActor func editorRejectsUnrelatedAppsAndSymlinks() async throws {
    let f = try Fixture()
    let config = try DestinationConfiguration(destination: f.destination, displayName: "Original")
    let directory = f.root.appendingPathComponent("Apps")
