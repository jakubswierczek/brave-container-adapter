...
The v1.1.1 ZIP is Developer ID signed and notarized. The older v1.1.0 ZIP is ad-hoc signed. Developer ID releases use the `-notarized.zip` filename. An individual or company Apple Developer Program membership can provide Developer ID signing, but membership alone does not install a certificate or grant certificate creation rights.
